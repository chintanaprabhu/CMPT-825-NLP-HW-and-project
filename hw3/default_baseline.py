...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, character_vector_dim):
        torch.manual_seed(1)
        ...
        self.lstm = nn.LSTM(embedding_dim + character_vector_dim, hidden_dim, bidirectional=False)
    ...
    def forward(self, sentence, character_vectors):
        ...
        features = torch.cat((embeds, character_vectors), 1)
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), character_vector_dim=300)
...
if __name__ == '__main__':
    ...
    optparser.add_option("-m", "--modelfile", dest="modelfile", default=os.path.join('data', 'chunker_baseline'), help="filename without suffix for model files")
