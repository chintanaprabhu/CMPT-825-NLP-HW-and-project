...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, character_vector_dim, character_rnn_hidden_dim):
        torch.manual_seed(1)
        super(LSTMTaggerModel, self).__init__()
        self.hidden_dim = hidden_dim
        self.character_rnn_hidden_dim = character_rnn_hidden_dim

        self.word_embeddings = nn.Embedding(vocab_size, embedding_dim)
        self.character_rnn = CharacterRNN(character_vector_dim, character_rnn_hidden_dim)

        # The LSTM takes word embeddings as inputs, and outputs hidden states
        # with dimensionality hidden_dim.
        self.lstm = nn.LSTM(embedding_dim + character_rnn_hidden_dim, hidden_dim, bidirectional=False)

        # The linear layer that maps from hidden state space to tag space
        self.hidden2tag = nn.Linear(hidden_dim, tagset_size)

    def forward(self, sentence, character_vectors):
        embeds = self.word_embeddings(sentence)
        character_rnn_hidden = self.character_rnn(character_vectors.view(len(sentence), 1, -1), torch.zeros(len(sentence), 1, self.character_rnn_hidden_dim))
        features = torch.cat((embeds, character_rnn_hidden.squeeze(1)), 1)
        lstm_out, _ = self.lstm(features.view(len(sentence), 1, -1))
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), character_vector_dim=300, character_rnn_hidden_dim=64)
    ...
    def argmax(self, seq):
        ...
        with torch.no_grad():
            ...
            character_vectors = prepare_character_vectors(seq)
            tag_scores = self.model(inputs, character_vectors)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                character_vectors = prepare_character_vectors(sentence)
                targets = prepare_sequence(tags, self.tag_to_ix, self.unk)

                # Step 3. Run our forward pass.
                tag_scores = self.model(sentence_in, character_vectors)
